...
Account-level analytics, aggregated across all your servers, are available from the **Analytics** item in the dashboard sidebar. Per-server analytics, scoped to a single server, are available on the **Analytics** tab of that server's detail page.
...
This is the clearest signal of which tools the AI is actually using. In practice you'll usually find that 80% of calls go to 2 or 3 tools, so invest optimisation effort there.
...
Use this to:
- **Find tools with zero calls**: bad description, or an unused feature?
