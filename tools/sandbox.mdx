---
title: "Tool Runtime"
description: "Every MCPCore tool runs in an isolated JavaScript runtime with a built-in SDK for HTTP, databases, and utilities."
---
...
Connect to a relational database and run queries. `sdk.db()` is powered by **[Knex.js](https://knexjs.org)** under the hood, a battle-tested SQL query builder for Node.js. If you're familiar with Knex, the API will feel natural. MCPCore wraps it with a simplified interface for the most common operations.
...
<Warning>
  Always use parameterised queries (`$1`, `$2`, and so on) when embedding user input in SQL. Never concatenate `params.*` values directly into a SQL string.
</Warning>
